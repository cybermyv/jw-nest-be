...
export class ProductService {
    ...
    constructor (  
        ...
        public async getProductByGroup(jwgroup: number): Promise<Product[]> {
            ...
            else { return await this.productRepository.find({
        ...
        public async addImageLink (id_product: number, path: string): Promise<IMGLink> {
            // eslint-disable-next-line @typescript-eslint/camelcase
            return await this.productRepository.query('insert into img_link (id_product, path) values(?, ?)', [id_product, path]);
